...
def act(ct: EasterCanvas):
    ...
    leave_colors = ['green'] + ['yellow'] * 10
    leave_fill = len(leave_colors) - 1
    ...
    def roses():
        for r in range(rose_number):
            ...
            for leave_xpos in [leaves_xpos,-leaves_xpos]:
                ...
                ct.step_to(leave_xpos + leave_ypos, move=True, color=leave_colors[0])
                for yfac in [1, -1]:
                    ...
                    shapes.heart(
                        ...
                        heart_turn = turn,
                        heart_fill = leave_fill if leave_xpos * yfac > 0 else 1,
