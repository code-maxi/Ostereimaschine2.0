...
def act(ct: EasterSimulator):
    ...
    music_x_offset = - (music_lines-1) * music_distance / 2 * 1.5
    
    note_number = 20
    note_distance = round(ct.egg_y_steps / note_number) * 1j

    note_dot_rad = music_distance/2 + ct.x_to_ysteps(music_distance/2 * 1.5)
            ...
            if value <= 1/2:
                ...
                if value == 1/8 and y.imag + note_distance.imag <= ct.egg_y_steps:
                    ...
                    for i in range(note_beam_width):
                        ct.step_to(ct.x_stroke_steps(), rel=True)
                        ct.step_to(delta * (1 if i % 2 == 0 else -1), rel=True)
                    
                    ct.step_to(-note_beam_width * ct.x_stroke_steps() - note_stem_length, rel=True)
