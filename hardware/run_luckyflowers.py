...
def act(ct: EasterCanvas):
    ...
    flower_subsize = (50 + flower_thickness * 166j) * 2.25

    flower_fill = 100
    flower_halffill = 0#flower_fill - 1
    flower_xpos = 0
    #flower_dot_radius = flower_size * 0.2

    lucky_clover_colors = ['green'] * 1 + ['orange'] * 2 + ['green'] * 2 + ['black'] * 10

    line_spacing = 1.5 * ct.x_velocity
    line_colors = [ 'green', 'orange', 'green' ]
    line_sin_width = ct.egg_xborder_steps/4 - flower_size/2 - line_spacing
    line_sin_number = 10
    line_sin_res = 32
    line_sin_offset = 2 * ct.y_velocity
    ...
    def flowers():
        for f in range(flower_number):
            ypos = (f / flower_number+0.5) * ct.egg_y_steps * 1j
            ct.step_to(flower_xpos + ypos, move=True)
            turn = random.random() * 2 * math.pi
